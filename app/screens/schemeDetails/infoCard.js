...
const InfoCard = ({data, pendingMonths}) => {
  const {height, width} = useWindowDimensions();
  console.log(data, 'infocard');
  const font = useWindowDimensions().fontScale;
  return (
    <View
      style={[styles.container, {height: height * (19 / 100), width: '100%'}]}>
      <View style={styles.divs}>
        <Text style={styles.titleOne}>Scheme Name</Text>
        <Text style={styles.titleTwo}>{data?.scheme_name}</Text>
      </View>
      <View style={[styles.divs]}>
        <Text style={[styles.titleOne, {alignSelf: 'flex-end'}]}>
          Total Amount
        </Text>
        <Text style={[styles.titleTwo, {alignSelf: 'flex-end'}]}>
          ₹{data?.total_amount}
        </Text>
      </View>
      <View style={styles.divs}>
